...
RSpec.describe 'RSpec Configuration' do
  it 'configures the API endpoint' do
    expect(RSpec.configuration.api_endpoint).to eq('https://api.example.com')
  end
  ...
  it 'configures the API credentials' do
    expect(RSpec.configuration.api_creds).to eq({ user: { email: 'your_username', password: 'your_password' } })
  end
end
